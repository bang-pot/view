...
## Meeting Round 08A 완료된 모임 히스토리 목록 + 방탈로그 연결

- `/crews/{crewId}/history/meetings`
  - 로그인한 현재 크루 ACTIVE 멤버만 접근할 수 있는 완료된 모임 히스토리 화면을 추가했습니다.
  - `GET /api/crews/{crewId}/history/meetings`를 `page`, `size` 기준으로 연결했습니다.
  - 비로그인 사용자는 보호 라우트 흐름에 따라 로그인 / 완료 프로필 화면으로 이동합니다.
  - 크루원이 아닌 사용자는 `/crews/public/{crewId}`로 되돌립니다.
- 히스토리 카드
  - 카드에는 테마명, 모임 제목, 장소, 날짜, 결과, 내 기록 상태를 표시합니다.
  - `myLogStatus`가 `HAS_LOG`면 `기록 보기`, `NO_LOG`면 `기록 작성` CTA를 보여줍니다.
  - `HAS_LOG`는 기존 `/logs/{logId}` 상세로 연결하고, `NO_LOG`는 기존 `/crews/{crewId}/meetings/{meetingId}/log` 작성 흐름으로 연결합니다.
- 추가 로딩
  - 이번 라운드는 `pageInfo.hasNext` 기반 `더 보기` 버튼으로 다음 페이지를 불러옵니다.
  - 기본 `size`는 20으로 고정했습니다.
- 크루 허브 연결
  - 크루 허브 내비게이션에 `완료 히스토리` 링크를 추가해서 방탈로그 피드와 별개로 meeting 관점의 기록 입구를 열었습니다.
- 범위 제한
  - 이번 라운드는 완료된 모임 히스토리 목록과 `기록 보기 / 기록 작성` 연결까지만 엽니다.
  - 새 history 상세, 검색/필터, 통계, 알림, 댓글/좋아요는 아직 열지 않습니다.

### Meeting Round 08A 자동 검증

- `npm.cmd run test -- src/test/shared/meeting-client.test.tsx src/test/app/crew-page.test.tsx src/test/app/crew-meeting-history-page.test.tsx`
- `npm.cmd run lint`
- `npm.cmd run test`
- `npm.cmd run build`

### Meeting Round 08A 수동 검증

- 로그인한 ACTIVE 크루원으로 `/crews/{crewId}/history/meetings`에 진입했을 때 히스토리 화면이 정상적으로 열리는 것을 확인했습니다.
- 카드에 테마명, 모임 제목, 장소, 날짜, 결과, 내 기록 상태가 자연스럽게 표시되는 것을 확인했습니다.
- `myLogStatus = HAS_LOG`인 경우 `기록 보기`로 기존 `/logs/{logId}` 상세 화면에 이동하는 것을 확인했습니다.
- `myLogStatus = NO_LOG`인 경우 `기록 작성`으로 기존 `/crews/{crewId}/meetings/{meetingId}/log` 작성 화면에 이동하는 것을 확인했습니다.
- `더 보기` 버튼으로 다음 페이지를 이어서 불러올 수 있고, 마지막 페이지에서는 `여기까지 모두 확인했어요.` 문구가 보이는 것을 확인했습니다.
- 비로그인 사용자는 기존 보호 라우트 흐름으로, 크루원이 아닌 사용자는 `/crews/public/{crewId}`로 분기되는 것을 확인했습니다.
- 크루 허브 내비게이션의 `완료 히스토리` 링크로 히스토리 화면에 진입할 수 있는 것을 확인했습니다.
- 새 history 상세, 검색/필터, 통계, 알림, 댓글/좋아요 UI가 노출되지 않는 것을 확인했습니다.
