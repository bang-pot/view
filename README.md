...
## Auth Round 11 프로필 달력(활동 스케줄)

- `/profile`
  - 기존 프로필 허브 구조는 유지하고, 활동 카드 아래에 `달력` 섹션을 추가했습니다.
  - 프로필 허브 bootstrap은 계속 `GET /api/users/me`를 기준으로 동작하고, 달력 데이터는 별도 `GET /api/users/me/calendar`로 읽습니다.
  - 따라서 달력 섹션이 실패해도 프로필 허브 전체는 유지됩니다.
- 달력 섹션 구성
  - 왼쪽은 월간 캘린더 그리드입니다.
  - 오른쪽은 선택한 날짜의 일정 리스트 패널입니다.
  - 기본 선택 규칙:
    - 현재 달에 일정이 있으면 현재 달을 먼저 보여줍니다.
    - 현재 달 일정이 없으면 첫 일정이 있는 달을 보여줍니다.
    - 일정이 전혀 없으면 현재 달과 오늘 날짜를 기본값으로 사용합니다.
- 일정 표시 방식
  - 달력 셀에는 날짜 숫자와 함께 일정 개수 또는 취소 일정 개수를 요약해 표시합니다.
  - 오른쪽 패널 일정 항목에는 날짜, 시간, 모임 제목, 크루명, 상태, 역할을 표시합니다.
  - 항목 클릭 시 기존 meeting detail `/crews/{crewId}/meetings/{meetingId}`로 이동합니다.
- 역할 / 상태 / 취소 표시
  - `participationRole`
    - `HOST` → `모임장`
    - `PARTICIPANT` → `참여자`
  - `meetingStatus`
    ...
    - `COMPLETED` → `완료`
    - `CANCELED` → `취소됨`
  - `isCanceled = true`인 일정은 회색조 카드와 `취소` 라벨로 구분합니다.
- 빈 상태 / 로딩 / 에러
  - 일정이 전혀 없으면 섹션 설명에서 `아직 표시할 일정이 없어요`를 보여줍니다.
  - 선택한 날짜에 일정이 없으면 오른쪽 패널은 유지한 채 `해당 날짜에는 일정이 없어요`를 보여줍니다.
  - 달력 섹션 로딩 문구는 `달력 정보를 불러오는 중입니다.`
  - 달력 섹션 에러 문구는 `달력 정보를 불러오지 못했어요. 잠시 후 다시 시도해 주세요.`
  - 에러 시 `다시 시도` 버튼으로 달력 섹션만 재요청합니다.
- 범위 제한
  - 이번 라운드는 활동 스케줄 읽기 전용 UI까지만 다룹니다.
  - 방팟로그 리스트, 찜 요약, 일정 생성/수정/삭제 기능은 열지 않습니다.
...
- 로그인한 `FULL` 사용자로 `/profile`에 진입했을 때 기존 프로필 허브가 유지된 상태에서 `달력` 섹션이 추가로 보이는 것을 확인했습니다.
- 달력 섹션의 왼쪽에는 월간 캘린더, 오른쪽에는 선택 날짜의 일정 리스트가 보이는 것을 확인했습니다.
- 내가 만든 모임과 내가 참가 확정된 모임이 모두 달력 대상에 포함되는 것을 확인했습니다.
- 특정 날짜를 클릭하면 오른쪽 패널이 해당 날짜 기준으로 갱신되고, 일정이 없는 날짜를 클릭해도 패널은 유지된 채 `해당 날짜에는 일정이 없어요`가 보이는 것을 확인했습니다.
- 일정 항목에서 날짜, 시간, 모임 제목, 크루명, 역할 라벨(`모임장` / `참여자`)이 자연스럽게 보이는 것을 확인했습니다.
- 취소된 모임은 회색 처리와 `취소` 라벨로 일반 일정과 명확히 구분되는 것을 확인했습니다.
- 일정 항목 클릭 시 기존 meeting detail `/crews/{crewId}/meetings/{meetingId}`로 이동하는 것을 확인했습니다.
